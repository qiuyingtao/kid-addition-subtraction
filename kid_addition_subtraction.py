...
class Controller(cmd.Cmd):
    ...
    def do_D(self, arg):
        ...
        if arg != EASY and arg != NORMAL and arg != HARD and arg != NIGHTMARE and arg != INFERNO:
            self.output(red('请输入e(10以内)或者n(20以内)或者h(100以内)或者m(20以内连加连减)或者i(100以内连加连减)!'))
            return
        self.difficulty = arg
        if arg == EASY:
            ...
            self.output('已把本次训练设为' + purple('100以内') + '的加法或减法')
        elif arg == NIGHTMARE:
            self.output('已把本次训练设为' + purple('20以内连续') + '的加法或减法')
        else:
            self.output('已把本次训练设为' + purple('100以内连续') + '的加法或减法')
        return
